...
export function Phase9Summary() {
  const phase9Words = vocabularyService.getPhase9Words()

  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-2xl">Phase 9 Vocabulary Expansion</CardTitle>
        <CardDescription>10 new words added (words 141-150)</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant="outline">Divine Actions</Badge>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <div>
                ...
                {phase9Words.filter((w) => w.difficulty === "beginner").length} words
              </div>
              <div>
                <span className="font-medium">Intermediate:</span>{" "}
                {phase9Words.filter((w) => w.difficulty === "intermediate").length} words
              </div>
              <div>
                <span className="font-medium">Advanced:</span>{" "}
                {phase9Words.filter((w) => w.difficulty === "advanced").length} words
              </div>
            </div>
          </div>
        </div>
