...
export const quranicAyatsData: QuranicAyat[] = [
  ...
  },
  {
    ...
    theme: "Tawhid",
    difficulty: "medium",
    tags: ["tawhid", "Allah", "unity"],
  },
]

// Function to get random ayats for different difficulty levels
export function getRandomAyatsByDifficulty(difficulty: "easy" | "medium" | "hard", count = 5): QuranicAyahPart[][] {
  const ayats = quranicAyatsGameData[difficulty]
  const shuffled = [...ayats].sort(() => Math.random() - 0.5)
  return shuffled.slice(0, count)
}

export function getRandomAyats(count = 5): QuranicAyat[] {
  const shuffled = [...quranicAyatsData].sort(() => Math.random() - 0.5)
  return shuffled.slice(0, count)
}

export function getAyatsByDifficulty(difficulty: "easy" | "medium" | "hard"): QuranicAyat[] {
  return quranicAyatsData.filter((ayat) => ayat.difficulty === difficulty)
}

export function getAyatsBySurah(surahNumber: number): QuranicAyat[] {
  return quranicAyatsData.filter((ayat) => ayat.surahNumber === surahNumber)
}

export function getAyatsByTheme(theme: string): QuranicAyat[] {
  return quranicAyatsData.filter((ayat) => ayat.theme.toLowerCase() === theme.toLowerCase())
}
