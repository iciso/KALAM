...
const quranicAyatsGameData: QuranicGameData = {
  ...
  hard: [
    ...
    [
      ...
      { id: "h3-2", text: "دعاء", translation: "the invocation of" },
...
// Convert the game data into QuranicAyat format and export getRandomAyats function
function convertToQuranicAyats(): {
  easy: QuranicAyat[]
  medium: QuranicAyat[]
  hard: QuranicAyat[]
} {
  const convertLevel = (
    ayats: typeof quranicAyatsGameData.easy,
    orders: string[][],
    difficulty: "easy" | "medium" | "hard",
  ): QuranicAyat[] => {
    return ayats.map((ayatParts, index) => {
      const correctOrder = orders[index] || []
      const surahNum = difficulty === "easy" ? index + 1 : difficulty === "medium" ? index + 1 : index + 1
      const ayatNum = index + 1

      return {
        id: `${surahNum}-${ayatNum}`,
        surah: surahNum,
        ayat: ayatNum,
        words: ayatParts.map((part) => ({
          id: part.id,
          text: part.text,
        })),
        translation: ayatParts.map((part) => part.translation).join(" "),
      }
    })
  }

  return {
    easy: convertLevel(quranicAyatsGameData.easy, quranicAyatsGameData.correctOrders.easy, "easy"),
    medium: convertLevel(quranicAyatsGameData.medium, quranicAyatsGameData.correctOrders.medium, "medium"),
    hard: convertLevel(quranicAyatsGameData.hard, quranicAyatsGameData.correctOrders.hard, "hard"),
  }
}

const convertedAyats = convertToQuranicAyats()

export function getRandomAyats(count: number, difficulty: "easy" | "medium" | "hard"): QuranicAyat[] {
  const ayatsPool = convertedAyats[difficulty]
  const shuffled = [...ayatsPool].sort(() => Math.random() - 0.5)
  return shuffled.slice(0, Math.min(count, ayatsPool.length))
}
